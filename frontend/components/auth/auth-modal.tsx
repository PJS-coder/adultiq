...
export default function AuthModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === 'signup') {
        ...
        if (response.success) {
          onOpenChange(false)
          if (response.user.quizCompleted) {
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[420px] p-8 bg-white">
        {/* Header */}
        <div className="mb-6">
          ...
          <p className="text-sm text-gray-500">
            {mode === 'login' ? 'Please enter your details.' : 'Please provide your details.'}
          </p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="space-y-1.5">
            ...
            <Input
              id="email"
              type="email"
              placeholder="dmitriycowell@gmail.com"
              value={email}
          ...
          >
            {loading ? 'Loading...' : mode === 'login' ? 'Sign in' : 'Continue'}
          </Button>
